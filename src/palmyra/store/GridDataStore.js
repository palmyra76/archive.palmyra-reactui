...
class GridDataStore {
  ...
  query(params, callback, options) {
    const targetUrl = this.baseUrl + this.target;
    var requestParams = {
      params: { ...params.page, ...params.filter },
      headers: { sortOrder: this.constructSortOrder(params.sortOrder) }
    }
    axios.get(targetUrl, requestParams, options)
      .then(response => {
  ...
  constructSortOrder(sortColumn) {
    var result = [];
    for (var key in sortColumn) {
      result.push((sortColumn[key] == 'asc' ? '+' : '-') + key);
    }
